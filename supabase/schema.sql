...
create table if not exists public.heat_lanes (
  ...
  -- relay_squad_id is added further down, immediately after public.relay_squads
  -- exists — that table is defined ~3000 lines below this one, so a foreign
  -- key here would break every from-scratch build.
...
-- ---------------------------------------------------------------------------
-- A heat lane may hold a relay SQUAD instead of an individual entry.
--
-- Declared here rather than in the heat_lanes CREATE TABLE above, because
-- public.relay_squads does not exist yet at that point in this file and the
-- foreign key would fail on a from-scratch build.
--
-- WHY A SQUAD ON THE LANE, AND NOT A SYNTHETIC ENTRY.
-- A relay has no public.entries row and must not be given one: an entry is
-- one swimmer in one race, and it is what pricing, the athlete event limit
-- and event capacity all count. Four fake entries per squad would bill the
-- swimmers individually for a squad the captain already paid for as a unit,
-- and would push each of them toward their event cap.
--
-- Hanging the squad off the lane instead means public.results attaches to a
-- relay exactly as it does to an individual swim — same table, same publish
-- flow, same referee screen — which is what lets a referee time a relay at
-- all. Before this, relay squads existed in relay_squads/relay_legs and
-- nowhere else: no heat, no lane, no result, and so no way to record one.
-- ---------------------------------------------------------------------------
alter table public.heat_lanes
...
--
-- Relays are seeded from public.relay_squads, not from entries, so
-- generate_heats_for_event() declines them outright (it returns 0 for any
-- is_relay event). This is the relay equivalent.
--
-- Only COMPLETE squads are seeded: four legs, and not on an expired hold. A
-- squad with three swimmers is not a competitor yet, and putting it in a lane
-- would give the referee a card that cannot be timed.
--
-- Heat gender comes from the EVENT, not from the swimmers. A relay event is
-- already gendered by name — "(Male)", "(Female)", or mixed — and a mixed
-- squad has no single gender, so heats.gender is left NULL there. That column
-- has always been nullable for exactly this kind of case.
--
-- Same "never rebuild a scored event" guard as the individual path: deleting
-- heats would cascade away results that have already been recorded.
-- ---------------------------------------------------------------------------
...
drop trigger if exists generate_relay_heats_on_leg_delete on public.relay_legs;
create trigger generate_relay_heats_on_leg_delete
  after delete on public.relay_legs
